...
export default function PhysicsChaptersPage() {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            try {
                const res = await fetch('/api/questions?subject=Physics&exam=KEAM');
                if (res.ok) {
            ...
            {questions.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-border bg-card/50 p-12 text-center">
                    <div className="text-muted-foreground">
                        ...
                        <p className="text-sm mt-1">Questions for Physics will be added soon.</p>
                    </div>
                </div>
            ) : (
                <div className="grid lg:grid-cols-4 gap-6">
                    ...
                    <div className="lg:col-span-3 space-y-3">
                        {filteredQuestions.map((question, index) => (
                            <Link
                                key={question.id}
                                href={`/keam/chapterwise/physics/${question.id}`}
